...
export default class Image {
  ...
  static fromMean(vol: Vol, depth = 0, scale = 1, normalize = false) {
    return Image.fromVol(vol, 0, [0,1,2], scale, normalize);
  }

  static fromFilter(vol: Vol, depth = 0, scale = 1, normalize = true) {
  ...
  static fromVol(vol: Vol, mean: any, channel: number | number[] = [2,1,0], scale = 1, normalize = false, aplha = 255){
    var img = new Image();
    ...
    for (let y=0; y < h; y++){
      for (let x=0; x < w; x++){
        let pp = (y*w + x) * 4;
        let mean_0 = mean ? mean instanceof Vol ? mean.get(x,y,c0) : +mean[c0] : 0;
        let mean_1 = mean ? mean instanceof Vol ? mean.get(x,y,c1) : +mean[c1] : 0;
        let mean_2 = mean ? mean instanceof Vol ? mean.get(x,y,c2) : +mean[c2] : 0;
  ...
  toVol(mean: any, channel = [2,1,0]) {
    ...
    var c1 = channel[1];
    var c2 = channel[2];
    var vol = new Vol(w, h, 3, 0.0);
    for (var y=0; y < h; y++){
      for (var x=0; x < w; x++){
        var pp = (y*w + x) * 4;
        var mean_0 = mean instanceof Vol ? mean.get(x,y,c0) : +mean[c0];
        var mean_1 = mean instanceof Vol ? mean.get(x,y,c1) : +mean[c1];
        var mean_2 = mean instanceof Vol ? mean.get(x,y,c2) : +mean[c2];
