...
(function(global) {
  ...
  var ConvLayer = function(opt) {
    ...
    if (typeof opt.filters !== 'undefined'){
      var n = this.sx * this.sy * this.in_depth;
      for(var i=0;i<this.out_depth;i++) {
        var vol = this.filters[i];
        vol.w = global.zeros(n);
        vol.dw = global.zeros(n);
        // copy over the elements.
        for(var j=0;j<n;j++) {
          vol.w[j] = opt.filters[i][j];
        }
      }
    }

    if (typeof opt.biases !== 'undefined') {
      this.biases = new Vol(1, 1, this.out_depth, bias);
      this.biases.fromJSON(opt.biases);
    }
...
(function(global) {
  ...
  global.ConcatLayer = ConcatLayer;
})(convnetjs);
(function(global) {
  "use strict";
  var Vol = global.Vol; // convenience
